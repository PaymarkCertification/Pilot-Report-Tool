...
def process_rpt_files():
    def concatentate_csvs(list_of: list) -> pd.DataFrame:
        ...
        try:
            ...
            dframe['Card Number'] = dframe['Card Number'].apply(str)
            dframe['Terminal'] = dframe['Terminal'].apply(str)
            dframe['Resp Code'] = dframe['Resp Code'].str.rstrip()
            dframe['Card Number'] = dframe['Card Number'].apply(lambda x: ps.scrub(x) if ps.is_full_pan(x) else x)
            # dframe.applymap(lambda x: x.strip() if isinstance(x, str) else x)
